...
:: 2. Create the custom named output batch file in a temp folder
:: (writing to the current folder can fail with Access Denied if this
:: script gets relaunched elevated, since UAC can change the working
:: folder to C:\Windows\System32 or similar)
set "WorkDir=%TEMP%\VpnDeployBuilder"
if not exist "!WorkDir!" mkdir "!WorkDir!"
set "OutputFile=!WorkDir!\deploy-!User!-!VpnName!-vpn.bat"

:: Anchor the letters to the folder this script was launched from,
:: not the current working directory (which can also change under UAC).
set "ScriptDir=%~dp0"
